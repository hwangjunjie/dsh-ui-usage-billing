...
const MODEL_CATALOG = [
	...
	// 腾讯云网关（llm-pi-ai provider: tencent-cloud / tokenhub.tencentmaas.com）
	// 中转模型：独立条目，归属显示「腾讯云」，费率暂参考官方价（如腾讯云实际
	// 定价不一致可单独调整）。不改动任何官方提供商条目。
	// tc/ 前缀条目：经腾讯云网关调用且模型 id 与官方条目相撞时使用（聚合层按
	// provider 加前缀），官方直连不受影响。
	{
		key: "tc/mimo-v2.5-pro",
		name: "MiMo V2.5 Pro",
		provider: "腾讯云",
		colorVar: "dsw-static-green-400",
		price: {
...
function foldSession(events, subscriptionProviders) {
	...
	for (const event of events) {
		...
		if (event.type === "request/header") {
			...
			// 腾讯云网关（tokenhub.tencentmaas.com）中转：模型 id（含别名）与官方
			// 条目相撞时改用 tc/ 前缀归入腾讯云专属条目——经腾讯云调用归属显示
			// 「腾讯云」，直连官方（deepseek-official、未来直连小米/智谱等）仍显示
			// 官方厂商，互不冲突。不撞官方条目的腾讯云模型（hy3 等）保持原 id。
			const resolved = MODEL_KEY_ALIASES[model] ?? model;
			const hit = MODEL_CATALOG.find((entry) => entry.key === resolved);
			key = provider === "tencent-cloud" && hit !== void 0 && hit.provider !== "腾讯云" ? "tc/" + resolved : resolved;
